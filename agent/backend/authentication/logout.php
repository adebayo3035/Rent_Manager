...
try {
    logActivity("Client Logout request received");
    ...
    $sessionUserId = (string) $_SESSION['client_code'];
    ...
    logActivity("Processing logout for client {$sessionUserId}");

    /* ---------------------------
     * 4. Invalidate DB session
     * --------------------------- */
    $stmt = $conn->prepare("
        UPDATE client_active_sessions 
    ...
    logActivity("CLIENT LOGOUT ERROR ({$e->getCode()}): {$e->getMessage()}");
