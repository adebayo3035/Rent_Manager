...
try {
    ...
    // Verify ownership
    $check_query = "SELECT client_code FROM client_notifications WHERE notification_id = ?";
    ...
    if (!$notification || $notification['client_code'] !== $client_code) {
        json_error("Notification not found", 404);
    }
    
    // Mark as read
    $update_query = "UPDATE client_notifications SET is_read = 1, read_at = NOW() WHERE notification_id = ?";
