    ...
    const tenantManager = new DataManager({
        ...
        populateDetails: async function (tenant) {
            ...
            body.innerHTML = `
                ...
                <tr><td><strong>Tenant ID</strong></td><td><input type="text" id="edit_tenant_id" class="validate" data-type="text" value="${tenant.tenant_code
                }" readonly></td></tr>

                <tr><td><strong>First Name</strong></td><td><input type="text" id="edit_firstname" class="validate" data-type="text" value="${tenant.firstname
                }"></td></tr>

                <tr><td><strong>Last Name</strong></td><td><input type="text" id="edit_lastname" class="validate" data-type="text" value="${tenant.lastname
                }"></td></tr>

                <tr><td><strong>Email</strong></td><td><input type="email" id="edit_email" class="validate" data-type="email" value="${tenant.email
                }"></td></tr>

                <tr><td><strong>Phone</strong></td><td><input type="number" id="edit_phone" value="${tenant.phone
                }" class="validate"
                        data-type="phone"></td></tr>

                <tr><td><strong>Gender</strong></td>
                    <td>
                        <select id="edit_gender" class="validate" data-type="select">
                ...
                <tr><td><strong>Allocated Property</strong></td>
                    <td>
                        <select id="edit_tenantProperty" data-type="select" class="select2 validate"></select>
                    </td>
                </tr>

                <tr><td><strong>Allocated Apartment</strong></td>
                    <td>
                        <select id="edit_tenantApartment" class="select2 validate" data-type = "select" disabled></select>
                    </td>
                </tr>

                <tr><td><strong>Payment Frequency</strong></td>
                    <td>
                        <select id="edit_rent_frequency" class="validate" data-type="select">
                            
                            <option value="Monthly" ${tenant.payment_frequency === "Monthly"
                    ? "selected"
                    : ""
                }>Monthly</option>
                            <option value="Quarterly" ${tenant.payment_frequency === "Quarterly"
                    ? "selected"
                    : ""
                }>Quarterly</option>
                            <option value="Semi-Annually" ${tenant.payment_frequency === "Semi-Annually"
                    ? "selected"
                    : ""
                }>Semi-Annually</option>
                            <option value="Annually" ${tenant.payment_frequency === "Annually"
                    ? "selected"
                    : ""
                }>Annually</option>
                        </select>
                ...
                <tr><td><strong>Lease Start</strong></td><td><input type="date" id="edit_lease_start" class="validate" data-type="date" value="${tenant.lease_start_date || ""
                }"></td></tr>
                <tr><td><strong>Lease End Date</strong></td><td><input type="date" id="edit_lease_end" class="validate" data-type="date" value="${tenant.lease_end_date || ""
                }"></td></tr>
                ...
                <tr><td><strong>Status</strong></td>
                    <td>
                        <select id="edit_status" class="validate" data-type="select">
                            <option value="1" ${tenant.status == 1 ? "selected" : ""
                }>Active</option>
                            <option value="0" ${tenant.status == 0 ? "selected" : ""
                }>Inactive</option>
                        </select>
                    </td>
                </tr>
            ...
            // Set property (this triggers apartment fetch)
            $("#edit_tenantProperty").val(tenant.property_code).trigger("change");

            // Wait a bit for apartments to load, then set apartment
            setTimeout(() => {
                $("#edit_tenantApartment")
                    .val(tenant.apartment_code)
                    .trigger("change.select2");
            }, 300);

            $("#edit_tenantProperty").on("change", async function () {
                const propertyCode = $(this).val();
                const apartmentSelect = $("#edit_tenantApartment");

                // Reset & lock apartment
                apartmentSelect
                ...
                apartmentSelect.html(`<option value="">Loading apartments...</option>`);
                ...
                try {
                    ...
                    apartmentSelect.html(`<option value="">-- Select Apartment --</option>`);

                    if (data.success && data.apartments?.length) {
                        data.apartments.forEach((apartment) => {
                            apartmentSelect.append(
                                new Option(
                                    apartmentLabelFormatter(apartment),
                                    apartment.apartment_code
                                )
                            );
                        });

                        apartmentSelect.prop("disabled", false).trigger("change.select2");
                    } else {
                        apartmentSelect.html(`<option value="">No available apartments</option>`);
                    }
                } catch (err) {
                    console.error("Fetch apartments failed:", err);
            ...
            $("#edit_tenantApartment").on("change", function () {
                if ($(this).val()) {
                    $(this).prop("disabled", true).trigger("change.select2");
                }
            });

            $("#edit_tenantProperty").on("change", function () {
            ...
            document
                .getElementById("updateTenantBtn")
                .addEventListener("click", () => {
                    UI.confirm("Are you sure you want to update this tenant?", () => {
                        this.updateItem(tenant.tenant_id, {
                            ...
                            apartment_code: document.getElementById("edit_tenantApartment").value,
                            ...
                            rent_payment_frequency: document.getElementById(
                                "edit_rent_frequency"
                            ).value,
...
function apartmentLabelFormatter(apartment) {
    let displayText = `Unit ${apartment.apartment_type_unit}`;
