...
async function getCurrentAdmin() {
    try {
        const response = await fetch('../backend/maintenance/get_current_admin.php');
                ...
                <div class="requests-grid">
                    ${requests.map(request => `
                        <div class="request-card priority-${request.priority}" onclick="viewRequestDetails(${request.request_id})">
                            ...
                            <div class="request-description">${escapeHtml(request.description.substring(0, 120))}${request.description.length > 120 ? '...' : ''}</div>
...
function toggleAssignedToMe() {
    showOnlyAssignedToMe = document.getElementById('assignedToMeToggle')?.checked || false;
    currentPage = 1;
    fetchMaintenanceRequests();
}
...
async function acceptRequest(requestId) {
    const confirmed = await showConfirmModal({
        ...
        message: 'Are you sure you want to accept this maintenance request? You will be responsible for resolving it.',
    ...
    try {
        const response = await fetch('../backend/maintenance/accept_maintenance_request.php', {
            method: 'POST',
...
async function rejectRequest(requestId) {
    const rejectionReason = await promptReason('Rejection Reason', 'Please provide a reason for rejecting this request:');
    ...
    const confirmed = await showConfirmModal({
        ...
        message: 'Are you sure you want to reject this request? It will be sent to Super Admin for reassignment.',
    ...
    try {
        const response = await fetch('../backend/maintenance/reject_maintenance_request.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
...
async function reassignRequest(requestId) {
    ...
    try {
        const response = await fetch('../backend/maintenance/get_available_admins.php');
        const data = await response.json();
        ...
        const reassignReason = await promptReason('Reassignment Notes', 'Reason for reassignment:', 'textarea');
        ...
        const reassignResponse = await fetch('../backend/maintenance/superadmin_reassign.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                request_id: requestId,
...
async function updateRequestStatus(requestId, newStatus) {
    ...
    if (newStatus === 'resolved') {
        resolutionNotes = await promptReason('Resolution Notes', 'Please provide details about how the issue was resolved:', 'textarea');
    ...
    try {
        const response = await fetch('../backend/maintenance/update_maintenance_status.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function showRequestDetailsModal(requestData) {
    ...
    console.log('Action Check:', {
        isAssignedToMe, isPending, isInProgress, hasAccepted, adminId,
    ...
    body.innerHTML = `
        <div class="detail-section">
            ...
            <div class="detail-grid">
                ...
                ${request.assigned_agent_name && request.assigned_agent_name !== 'Not assigned' ? `
                ...
                ` : ''}
                ${request.assigned_admin_name && request.assigned_admin_name !== 'Not assigned' ? `
