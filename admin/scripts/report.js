...
class QueryRunner {
  constructor() {
    ...
    if (missingElements.length > 0) {
      console.warn(
        "QueryRunner: Missing elements:",
        missingElements.map((e) => e.name),
      );
    } else {
    ...
    if (this.sqlQuery && this.runQueryBtn) {
      ...
      console.error(
        "QueryRunner: Essential elements missing. Cannot initialize.",
      );
      if (this.errorAlert) {
  ...
  async runQuery() {
    ...
    // const token_id = this.csrfTokenName;
    ...
    // Validate CSRF token exists
     // ✅ FIXED: Check if token exists OR fetch it
    if (!this.csrfToken) {
      this.showError("Loading security token...");
      await this.fetchCsrfToken(); // Wait for token
    ...
    try {
      ...
      const response = await fetch("../backend/staffs/report.php", {
        ...
        body: JSON.stringify({
          query: query,
           csrf_token: this.csrfToken,  // ✅ Use stored token
          token_id: this.csrfTokenName, // ✅ Use token ID
          format: "json",
        }),
      });
  ...
  exportToCSV() {
    console.log(
      "QueryRunner: exportToCSV called, currentResults:",
      this.currentResults,
    );

    if (
      !this.currentResults ||
      !this.currentResults.columns ||
