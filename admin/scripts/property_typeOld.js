class PropertyTypeManager {
  ...
  setupEditDeleteListeners() {
    ...
    document.querySelectorAll(".restore-icon").forEach((span) => {
      span.addEventListener("click", () =>{
        const classType = span.getAttribute("data-class-type");
        const classId = span.getAttribute("data-class-id");

        if (classType !== "property_type") return;
        UI.confirm("Are you sure you want to Restore this Property Type?", () =>
          this.restorePropertyType(classId)
        )
      }
  ...
  async restorePropertyType(propertyTypeId) {
    // if (!confirm("Are you sure you want to delete this Property Type?")) return;
    ...
    try {
      ...
      if (data.success) {
        UI.alert("Property Type has been successfully Restored!", " Success");
        this.fetchPropertyTypes(this.currentPage);
      } else {
        UI.alert("Failed to Restore Property Type: " + data.message, " Error");
      }
    } catch (error) {
      console.error("Error Restoring Property Type:", error);
      UI.alert("Error Restoring Property Type", " System Error");
    }
  }
