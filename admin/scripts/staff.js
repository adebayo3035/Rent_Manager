    ...
    renderRow: function (staff, userRole) {
      ...
      if (staff.status == 1) {
        ...
        if (userRole == "Super Admin") {
          row += ` <td><span class="delete-icon" data-id="${staff.unique_id}">🗑️</span></td>`;
        } else {
        ...
        if (userRole === "Super Admin") {
          row += `
                <td colspan="2" style="text-align:center;"><span class="restore-icon" data-id="${staff.unique_id}">↻ Restore</span></td>
            `;
        } else {
          row += `
