...
document.addEventListener("DOMContentLoaded", () => {
  initFormValidation(
    ...
    "submitBtnAddApartmentType",
    "addApartmentTypeMessage",
  );
  ...
  const apartmentTypeManager = new DataManager({
    ...
    fetchDetailsUrl:
      "../backend/apartment_types/fetch_apartment_type_details.php",
    ...
    renderRow: function (item, userRole) {
      ...
      const status =
        item.status == 1
          ? '<span style="color: green; text-align:center;">Active</span>'
          : '<span style="color: red; text-align:center;">Inactive</span>';
      ...
      if (item.status == 1) {
        rowHTML += `
                    <td style="text-align:center;"><span class="edit-icon" data-id="${item.type_id}">✏️</span></td>`;
        if (userRole === "Super Admin") {
          rowHTML += `<td style="text-align:center;"><span class="delete-icon" data-id="${item.type_id}">🗑️</span></td>`;
        } else {
          rowHTML += `<td><span class="not-allowed-icon">⛔</span></td>`;
        }
      } else {
        if (userRole === "Super Admin") {
          rowHTML += `
    ...
    populateDetails: function (apartmentType) {
      ...
      const tableBody = document.querySelector(
        "#apartmentTypeDetailsTable tbody",
      );
      if (!tableBody) {
      ...
      document
        .getElementById("updateApartmentTypeBtn")
