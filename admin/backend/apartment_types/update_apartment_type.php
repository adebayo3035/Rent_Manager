...
try {
    ...
    if (!$input || json_last_error() !== JSON_ERROR_NONE) {
        logActivity("Invalid JSON input while updating apartment type by user $adminId");
    ...
    logActivity("REQUEST: Apartment type update | Action=$action_type | ID=$type_id | User=$adminId | Role=$adminRole");
    ...
    logActivity("TRANSACTION STARTED for apartment_type update (ID=$type_id)");

    // Check if type exists
    $check = $conn->prepare("SELECT type_id, status FROM apartment_type WHERE type_id = ? FOR UPDATE");
    if (!$check) {
    ...
    if (!$check->fetch()) {
        $check->close();
        $conn->rollback();
        logActivity("ERROR: Apartment type $type_id does not exist (User $adminId)");
    ...
    logActivity("TRANSACTION COMMITTED: Apartment type ID=$type_id updated by user $adminId");
...
function handleFullUpdate($conn, $type_id, $type_name, $description, $status, $adminId, $adminRole)
{
    ...
    if ($status == 0 && $adminRole !== "Super Admin") {
        ...
        json_error("You do not have permission to deactivate apartment types.", 403);
    }

    // Duplicate check (exclude current record and soft-deleted records)
    $dup = $conn->prepare("
        SELECT type_id FROM apartment_type 
    ...
    if ($dup->num_rows > 0) {
        ...
        json_error("Another apartment type already uses this name.", 409);
    }
    $dup->close();
    ...
    logActivity("SUCCESS: Full update for apartment type ID=$type_id | Affected rows: $affected");

    // Log the actual update for debugging
    logActivity("UPDATE DETAILS: ID=$type_id, name='$type_name', status=$status, by user=$adminId");

    return "Apartment type updated successfully!";
}
...
function handleStatusUpdate($conn, $type_id, $new_status, $adminId, $adminRole, $action)
{
    ...
    if ($adminRole !== "Super Admin") {
        ...
        json_error("You do not have permission to $action apartment types.", 403);
    }
    ...
    logActivity("SUCCESS: Apartment type ID=$type_id $action by user $adminId | Affected rows: $affected");

    return "Apartment type " . ($action === "delete" ? "deactivated" : "restored") . " successfully!";
}