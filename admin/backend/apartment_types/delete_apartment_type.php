...
if ($check->num_rows === 0) {
    logActivity("Delete failed: Apartment type $type_id not found");
...
if ($stmt->execute()) {
    logActivity("Soft deleted apartment type $type_id by user $deleted_by");
    json_success("Apartment type deleted successfully.", 200);
} else {
    logActivity("Delete failed: " . $stmt->error);
    json_error("Failed to delete apartment type.", 500);
}
