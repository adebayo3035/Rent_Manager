...
async function fetchApartmentDetails() {
  try {
    const response = await fetch(
      "../backend/tenant/fetch_apartment_details.php",
    );
...
function renderApartmentDetails() {
  ...
  const clientInfo = apartmentDetails.client_details || {};
  ...
  const html = `
        <div class="apartment-container">
            ...
            <div class="detail-card">
                <h3><i class="fas fa-user"></i> Property Owner Information</h3>
                
                    <div class="detail-item">
                        <span class="detail-label">Full Name:</span>
                        <span class="detail-value">${escapeHtml(clientInfo.client_name)}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Email:</span>
                        <span class="detail-value">${clientInfo.client_email || "N/A"}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Phone:</span>
                        <span class="detail-value">${clientInfo.client_phone || "N/A"}</span>
                    </div>
...
function downloadLeaseAgreement() {
  ...
  if (tenantCode) {
    window.open(
      `../backend/tenant/download_lease.php?tenant_code=${tenantCode}`,
      "_blank",
    );
  } else {
    if (window.showToast) {
      window.showToast(
        "Unable to download lease agreement. Tenant information not found.",
        "error",
      );
    }
  }
