...
function renderProfile() {
  ...
  // Determine if secret question tab should be shown
  // Show tab only if has_secret_set is 0 or false or not set
  const showSecretTab = !currentUser.has_secret_set || currentUser.has_secret_set === 0;
  
  // Build tabs HTML conditionally
  let tabsHtml = `
    <div class="tabs">
  ...
  if (showSecretTab) {
    tabsHtml += ` <div class="tab" onclick="switchTab('secret')">Secret Question</div>`;
  }
            ...
            <div class="profile-card">
                ...
                <div id="secretTab" class="tab-content">
                    <form class="profile-form" id="secretForm">
                        <div class="form-group">
                            ...
                            <select name="secret_question" id="secretQuestion" required>
                        ...
                        <button type="button" class="btn-save" onclick="setSecretAnswer()">
                            <i class="fas fa-key"></i> Set Secret Answer
                        </button>
                    </form>
                </div>
                ` : `
                <div id="secretTab" class="tab-content">
                    <div class="profile-form" style="text-align: center; padding: 40px;">
                        ...
                        <p style="color: #666; font-size: 14px;">If you need to change your security question, please contact support.</p>
                    </div>
                </div>
                `}
            </div>
...
function switchTab(tab) {
  ...
  if (tab === "personal") {
    ...
    const tabs = document.querySelectorAll(".tab");
    // Find the secret tab (could be at index 2 or different)
    for (let i = 0; i < tabs.length; i++) {
      if (tabs[i].innerText.includes("Secret")) {
...
// Rest of your existing functions (updateProfile, validatePassword, validatePasswordMatch, setSecretAnswer, changePassword, escapeHtml) remain the same
...
function validatePassword() {
  ...
  if (reqUpper) {
    ...
    reqUpper.textContent = /[A-Z]/.test(password)
      ? "✓ At least one uppercase letter"
      : "At least one uppercase letter";
  }
  if (reqLower) {
    reqLower.className = /[a-z]/.test(password) ? "valid" : "";
    reqLower.textContent = /[a-z]/.test(password)
      ? "✓ At least one lowercase letter"
      : "At least one lowercase letter";
  }
  if (reqNumber) {
    reqNumber.className = /[0-9]/.test(password) ? "valid" : "";
    reqNumber.textContent = /[0-9]/.test(password)
      ? "✓ At least one number"
      : "At least one number";
  }
...
async function setSecretAnswer() {
    ...
    if (secretAnswer !== confirmAnswer) {
        if (window.showToast) {
            window.showToast("Secret Answers do not match", "error");
        }
        return;
    }
    ...
    if (secretAnswer.length < 8) {
        if (window.showToast) {
            window.showToast("Secret Answer must be at least 8 characters", "error");
        }
        return;
    }

    // Show loading state on button
    const submitBtn = document.querySelector('#secretTab .btn-save');
    ...
    try {
        ...
        if (data.success) {
            if (window.showToast) {
                window.showToast(data.message || "Secret Question and Answer set successfully", "success");
            }
            const form = document.getElementById("secretForm");
            if (form) form.reset();
            
            // Update has_secret_set in currentUser
            if (window.currentUser) {
                window.currentUser.has_secret_set = 1;
            }
            
            // Wait 2 seconds so user can see the success message before reload
            setTimeout(() => {
                window.location.reload();
            }, 3000);
            
        } else {
            throw new Error(data.message || "Failed to set Secret Question and Answer");
        }
    } catch (error) {
        console.error("Error setting Secret Question and Answer:", error);
        if (window.showToast) {
            window.showToast(error.message, "error");
        }
