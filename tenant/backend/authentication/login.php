...
class LoginSecurity
{
    ...
    public function findUserByCredentials($username)
    {
        ...
        $stmt = $this->conn->prepare("SELECT * FROM tenants WHERE email = ? OR phone = ? OR tenant_code = ?");
        if (!$stmt) {
        ...
        $this->log("User found - ID: {$user['tenant_code']}, Status: {$user['status']}");
    ...
    public function checkLockoutStatus($userId)
    {
        ...
        $stmt = $this->conn->prepare("SELECT attempts, locked_until FROM tenant_login_attempts WHERE tenant_code = ?");
        if (!$stmt) {
