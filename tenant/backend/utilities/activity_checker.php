...
if (isset($_SESSION['tenant_code'])) {
    ...
    if (isset($_SESSION['last_activity'])) {
        ...
        // If idle time exceeds session timeout, call logout.php with tenant_code
        if ($idle_time > $session_timeout) {
            $tenant_code = $_SESSION['tenant_code'];

            // Prepare data to send in POST request
            $data = ['logout_id' => $tenant_code];
            $options = [
                'http' => [
            ...
            $response = file_get_contents("localhost/rent_manager/tenant/backend/logout.php", false, $context);
