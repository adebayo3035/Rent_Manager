...
LEFT JOIN clients c ON p.client_code = c.client_code  -- <-- NEW JOIN
...
LIMIT 1
    ...
    if ($apartment['lease_end_date']) {
        ...
        $today = new DateTime();
        $days_remaining = $today->diff($end_date)->days + 1;
        if ($today > $end_date) {
            $days_remaining = 0;
        }
    }
    ...
    $response_data = [
        ...
        'client_details' => [
            'client_name' => $apartment['client_name'],
            'client_email' => $apartment['client_email'],
            'client_phone' => $apartment['client_phone']
        ]
    ];
