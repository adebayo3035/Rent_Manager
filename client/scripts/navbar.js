...
function setupEventListeners() {
    ...
    const sidebar = document.getElementById('clientSidebar') || document.getElementById('tenantSidebar');
...
async function updateUserInfo() {
    ...
    const clientCode = user.client_code || user.client_code || 'Client ID Not Found';
    ...
    if (nameElement) {
        ...
        console.warn('clientName element not found in DOM');
    }
    
    // Update apartment number
    if (idElement) {
        idElement.textContent = `Client ID: ${clientCode}`;
        console.log('Client ID Updated to:', idElement.textContent);
    } else {
        console.warn('client ID element not found in DOM');
    }
...
function renderUserInitials(container, fullName) {
    ...
    const initials = (fullName || 'Client')
        .split(' ')
        .filter(Boolean)
        .slice(0, 2)
...
async function handleLogout() {
    ...
    console.log('Logging out user:', navbarCurrentUser.client_code);
    ...
    confirmBtn.onclick = async () => {
        ...
        try {
            const logoutData = {
                logout_id: navbarCurrentUser.client_code  // This should match what your backend expects
            };
