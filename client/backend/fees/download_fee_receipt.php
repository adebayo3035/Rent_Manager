<?php
// client/backend/fees/download_fee_receipt.php
...
try {
    ...
    // Get payment and fee details - verify ownership via client_code
    $query = "
        SELECT 
            p.*,
            tf.fee_type_id,
    ...
    $stmt->bind_param("is", $payment_id, $client_code);
    $stmt->execute();
    ...
    // Generate HTML for PDF - Clean and Professional
...
';
    ...
    $pdf->Output("Fee_Receipt_{$payment['receipt_number']}.pdf", 'I');
    
    exit();

} catch (Exception $e) {
    logActivity("Error in download_fee_receipt (client): " . $e->getMessage());
