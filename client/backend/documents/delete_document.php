<?php
// delete_document.php - Soft delete a document for the client and remove physical file
...
try {
    ...
    if (!isset($_SESSION['client_code'])) {
        logActivity("ERROR: No client code in session");
    ...
    logActivity("Step 2 - Client authenticated: {$client_code}");
    ...
    $query = "
        SELECT document_id, file_name, client_code, document_name, original_file_name
    ...
    if ($document['client_code'] !== $client_code) {
        logActivity("ERROR: Unauthorized delete attempt - Client: {$client_code}, Document Owner: {$document['client_code']}");
        json_error("Unauthorized access", 403);
    }
    ...
    $stmt->bind_param("sii", $client_code, $file_deleted_int, $document_id);
    ...
    createDocumentNotification($conn, $client_code, $document['document_name'], 'deleted');
