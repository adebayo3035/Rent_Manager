...
function verifyAndRehashPassword($pdo, $clientCode, $inputPassword, $storedHash) {
    ...
    if (password_needs_rehash($storedHash, PASSWORD_ARGON2ID)) {
        ...
        $stmt = $pdo->prepare("UPDATE clients SET password = ? WHERE client_code = ?");
        $stmt->execute([$newHash, $clientCode]);
    }
...
function verifyAndRehashSecretAnswer($pdo, $adminId, $inputSecretAnswer, $storedHash) {
    ...
    if (password_needs_rehash($storedHash, PASSWORD_ARGON2ID)) {
        ...
        $stmt = $pdo->prepare("UPDATE clients SET secret_answer = ? WHERE client_code = ?");
