...
logActivity("========== CLIENT DOWNLOAD RECEIPT - START ==========");

try {
    // Check authentication
    if (!isset($_SESSION['client_logged_in']) || !isset($_SESSION['client_code'])) {
        logActivity("ERROR: No client code in session or not logged in");
        header('Location: ../login.php');
        exit();
    }

    // CORRECT PATH to TCPDF - from client/backend/payments/ to root vendor folder
    $tcpdfPath = __DIR__ . '/../../../vendor/tecnickcom/tcpdf/VEND/tcpdf.php';
    ...
    if (!file_exists($tcpdfPath)) {
        ...
        $receipt_data = getReceiptData($conn, $client_code, $receipt_number, $tracker_id);
    ...
    logActivity("Client Code: {$client_code}, Receipt: {$receipt_number}, Tracker ID: {$tracker_id}");
    ...
    if (!$receipt_data) {
        logActivity("ERROR: Receipt not found for client: {$client_code}");
        $_SESSION['error'] = "Receipt not found.";
        header('Location: ../payments.php');
        exit();
    }
...
/**
 * Fetch receipt data from database for client
 */
/**
 * Fetch receipt data from database for client
 */
 ...
 * Fetch receipt data from database for client
 */
function getReceiptData($conn, $client_code, $receipt_number, $tracker_id) {
    $receipt_data = null;

    // PRIORITY 1: Find by tracker_id first (for period payments - MOST ACCURATE)
    // This includes ALL statuses (paid, approved, failed, pending_verification)
    if ($tracker_id) {
        $query = "
            SELECT 
                t.tracker_id,
                ...
                t.verified_by,
                t.admin_notes,
                t.created_at as tracker_created_at,
                t.updated_at as tracker_updated_at,
                r.rent_payment_id,
                r.receipt_number as parent_receipt_number,
                r.amount as annual_rent,
                r.payment_amount_per_period,
                r.payment_period,
                c.client_code,
                c.firstname,
                c.lastname,
                ...
                'period_payment' as receipt_type
            ...
            INNER JOIN clients c ON p.client_code = c.client_code
        ...
        $stmt->bind_param("is", $tracker_id, $client_code);
        $stmt->execute();
        ...
        if ($receipt_data) {
            // Generate a unique receipt number for this tracker record if it doesn't have one
            // Use tracker-specific receipt number format
            if (empty($receipt_data['parent_receipt_number'])) {
                $receipt_data['receipt_number'] = 'TRK-' . strtoupper(uniqid()) . '-' . $receipt_data['tracker_id'];
            } else {
                // Append tracker ID to make it unique for failed/partial payments
                $receipt_data['receipt_number'] = $receipt_data['parent_receipt_number'] . '-P' . $receipt_data['period_number'];
            }
            
            logActivity("Found receipt by tracker_id: {$tracker_id}, Status: {$receipt_data['status']}");
            return $receipt_data;
        }
    }

    // PRIORITY 2: Try to find by receipt_number in rent_payment_tracker
    if ($receipt_number && !$receipt_data) {
        $query = "
            SELECT 
                t.tracker_id,
                t.period_number,
            ...
            INNER JOIN clients c ON p.client_code = c.client_code
            LEFT JOIN agents ag ON p.agent_code = ag.agent_code
            WHERE (r.receipt_number = ? OR t.payment_reference = ?)
            AND p.client_code = ?
            ORDER BY t.tracker_id DESC
            LIMIT 1
        ";

        $stmt = $conn->prepare($query);
        $stmt->bind_param("sss", $receipt_number, $receipt_number, $client_code);
        $stmt->execute();
        $result = $stmt->get_result();
        $receipt_data = $result->fetch_assoc();
        $stmt->close();
        ...
        if ($receipt_data) {
            ...
            logActivity("Found receipt by receipt_number in rent_payment_tracker: {$receipt_number}");
            return $receipt_data;
        }
    }

    // PRIORITY 3: Check in rent_payments table (initial rent payment - no tracker record yet)
    if ($receipt_number && !$receipt_data) {
        $query = "
            SELECT 
                NULL as tracker_id,
                1 as period_number,
                r.period_start_date as start_date,
                r.period_end_date as end_date,
                ...
                r.created_at as tracker_created_at,
                r.updated_at as tracker_updated_at,
                r.rent_payment_id,
            ...
            INNER JOIN apartments a ON r.apartment_code = a.apartment_code
            INNER JOIN properties p ON a.property_code = p.property_code
            INNER JOIN clients c ON p.client_code = c.client_code
            LEFT JOIN agents ag ON p.agent_code = ag.agent_code
        ...
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ss", $receipt_number, $client_code);
        $stmt->execute();
        $result = $stmt->get_result();
        $receipt_data = $result->fetch_assoc();
        $stmt->close();
        ...
        if ($receipt_data) {
            logActivity("Found receipt by receipt_number in rent_payments: {$receipt_number}");
            return $receipt_data;
        }
    }

    // PRIORITY 4: Check in client_fees table
    if ($receipt_number && !$receipt_data) {
        $query = "
            SELECT 
                cf.fee_id as tracker_id,
                1 as period_number,
                cf.due_date as start_date,
                cf.due_date as end_date,
                cf.amount as amount_paid,
                cf.payment_date,
                cf.payment_method,
                cf.transaction_reference as payment_reference,
                cf.status,
                NULL as verified_at,
                NULL as verified_by,
                cf.notes as admin_notes,
                cf.created_at as tracker_created_at,
                cf.updated_at as tracker_updated_at,
                NULL as rent_payment_id,
                cf.receipt_number as receipt_number,
                cf.amount as annual_rent,
                cf.amount as payment_amount_per_period,
                'one_time' as payment_period,
                c.client_code,
                c.firstname,
                c.lastname,
                c.email,
                ...
                ag.phone as agent_phone,
                ag.email as agent_email,
                'fee_payment' as receipt_type
            FROM client_fees cf
            INNER JOIN clients c ON cf.client_code = c.client_code
            INNER JOIN apartments a ON cf.apartment_code = a.apartment_code
            INNER JOIN properties p ON a.property_code = p.property_code
            LEFT JOIN agents ag ON p.agent_code = ag.agent_code
            WHERE cf.receipt_number = ? 
            AND cf.client_code = ?
            LIMIT 1
        ";
        ...
        if ($receipt_data) {
            logActivity("Found receipt by receipt_number in client_fees: {$receipt_number}");
            return $receipt_data;
        }
    }

    // PRIORITY 5: Check in payments table (security deposit or other payments)
    if ($receipt_number && !$receipt_data) {
        $query = "
            SELECT 
                p.payment_id as tracker_id,
                1 as period_number,
                NULL as start_date,
                NULL as end_date,
                ...
                p.created_at as tracker_created_at,
                p.updated_at as tracker_updated_at,
                NULL as rent_payment_id,
                p.receipt_number as receipt_number,
                p.amount as annual_rent,
                p.amount as payment_amount_per_period,
                'one_time' as payment_period,
                c.client_code,
                c.firstname,
                c.lastname,
                ...
                ag.phone as agent_phone,
                ag.email as agent_email,
                p.payment_category as receipt_type
            FROM payments p
            INNER JOIN clients c ON p.client_code = c.client_code
            INNER JOIN apartments a ON p.apartment_code = a.apartment_code
            INNER JOIN properties pr ON a.property_code = pr.property_code
        ...
        if ($receipt_data) {
            logActivity("Found receipt by receipt_number in payments: {$receipt_number}");
            return $receipt_data;
        }
    }
...
function generateHTMLReceipt($data) {
    ...
    // Get the actual receipt number (use tracker-specific number)
    $display_receipt_number = $data['receipt_number'] ?? ($data['parent_receipt_number'] ?? 'N/A');
    ...
    } elseif ($receipt_type === 'fee_payment') {
        ...
        <div class="receipt-container">
            ...
            <div class="receipt-body">
                <div class="receipt-title">
                    <h2>' . $receipt_title . '</h2>
                    <p>Receipt No: ' . htmlspecialchars($display_receipt_number) . '</p>
                </div>
                ...
                <div class="info-grid">
                    <div class="info-box">
                        ...
                        <p><strong>Status:</strong> <span class="status-badge ' . $status_badge_class . '">' . $status_text . '</span></p>
                    </div>
                    <div class="info-box">
                        ...
                        <p><strong>Client Code:</strong> ' . htmlspecialchars($data['client_code']) . '</p>
                    </div>
                    <div class="info-box">
                    ...
                    <div class="info-box">
                        ...
                        <p><strong>Agent:</strong> ' . htmlspecialchars($data['agent_name'] ?? 'N/A') . '</p>
                        <p><strong>Phone:</strong> ' . htmlspecialchars($data['agent_phone'] ?? 'N/A') . '</p>
                        <p><strong>Email:</strong> ' . htmlspecialchars($data['agent_email'] ?? 'N/A') . '</p>
                    </div>
                </div>
    ...
    if ($receipt_type === 'security_deposit') {
        $html .= '<tr><td>Security Deposit for Apartment ' . htmlspecialchars($data['apartment_number']) . '</td>
                  <td class="amount">' . number_format($data['amount_paid'], 2) . '</td>
                </tr>';
    } elseif ($receipt_type === 'fee_payment') {
        $html .= '<td><td>Fee Payment</td>
                  <td class="amount">' . number_format($data['amount_paid'], 2) . '</td>
                </tr>';
    } elseif ($receipt_type === 'initial_rent') {
        $html .= '<tr><td>Initial Rent Payment</td>
                  <td class="amount">' . number_format($data['amount_paid'], 2) . '</td>
                </tr>';
    } else {
        $period_text = $period_display ? " ({$period_display})" : '';
        $html .= '<tr><td>Rent Payment - Period #' . $data['period_number'] . $period_text . '</td>
                  <td class="amount">' . number_format($data['amount_paid'], 2) . '</td>
                </tr>';
    }
    
    // Add admin notes if present (for failed/rejected payments)
    if (!empty($data['admin_notes'])) {
        $html .= '
                        <tr>
                            <td colspan="2" style="background: #fef2f2; padding: 10px;">
                                <strong>Notes:</strong> ' . htmlspecialchars($data['admin_notes']) . '
                            </td>
                        </tr>';
    }
    ...
    $html .= '
                ...
                <div class="footer">
                    ...
                    <p>&copy; ' . date('Y') . ' RentEase - All Rights Reserved</p>
                </div>
            </div>
        </div>
        <div class="no-print" style="text-align: center; margin-top: 20px;">
            <button onclick="window.print()" style="padding: 10px 20px; background: #1e3c72; color: white; border: none; border-radius: 8px; cursor: pointer;">
                🖨️ Print Receipt
            </button>
...
function generatePDFReceipt($data) {
    ...
    $pdf->SetCreator('RentEase');
    $pdf->SetAuthor('RentEase');
    ...
    <table width="100%" cellpadding="5">
        <tr>
            <td width="60%" style="border-bottom: 1px solid #333;">
                <h1 style="color: #1e3c72; margin: 0;">RentEase</h1>
    ...
    <table width="100%" cellpadding="5">
        <tr>
            <td width="50%" style="background-color: #f8f9fa;">
                ...
                Status: ' . htmlspecialchars($data['status'] ?? 'N/A') . '<br>
            ...
            <td width="50%" style="background-color: #f8f9fa;">
                ...
                Client Code: ' . htmlspecialchars($data['client_code']) . '
              </td>
        </tr>
    </table>
    ...
    <table width="100%" cellpadding="8" style="border-collapse: collapse;">
        <tr style="background-color: #1e3c72; color: white;">
            ...
            <th style="padding: 8px; text-align: right;">Amount (N)</th>
        </tr>';
    ...
    $html .= '
        <tr style="background-color: #f8f9fa; font-weight: bold;">
            ...
            <td style="padding: 8px; text-align: right; border-top: 2px solid #333;">N' . number_format($data['amount_paid'], 2) . '</td>
         </tr>
     </table>
        ...
        <tr>
            <td style="border-top: 1px solid #ddd; padding-top: 15px;">
                <p style="font-size: 11px; color: #666;">
                    ...
                    <strong>Payment Date:</strong> ' . date('F j, Y', strtotime($data['payment_date'])) . '<br>
                    <strong>Payment Method:</strong> ' . ucfirst(str_replace('_', ' ', $data['payment_method'])) . '<br>
                    <br>
        ...
        <tr>
            <td style="text-align: center;">
                <p style="font-size: 10px; color: #999;">
                    RentEase - Property Management System<br>
